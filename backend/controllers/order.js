...
exports.addCart = asyncHandler(async (req, res, next) => {
  ...
  const food = await Foods.findById(req.body._id);
  const { photo, title, rate, stock } = food;
...
exports.placeOrder = asyncHandler(async (req, res, next) => {
  ...
  if (order.user.toString() !== req.user.id && req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `User ${req.user.id} is not authorized to update food ${order._id}`,
        401
      )
    );
  }
...
exports.deleteCart = asyncHandler(async (req, res, next) => {
  ...
  if (order.user.toString() !== req.user.id && req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `User ${req.user.id} is not authorized to delete food ${order._id}`,
        401
      )
    );
  }
